...
```bash
python main_training.py --config config_dgppo_minimal.json
```

修订版从零训练，输出 `outputs/dgppo_minimal_v2/`，保留原失败实验 `outputs/dgppo_minimal/`。默认 `config.json` 及阶段 7/8A/8B/9 的算法分支保持不变。完整训练由用户执行。训练完成后，将 `main_testing.py` 的 `path` 或评估脚本的模型目录设为该输出目录；评估日志目录也需相应设置。本次没有改变用户的测试/评估路径。

无安全更新的对照使用 `python main_training.py --config config_dgppo_task_only.json`，输出 `outputs/dgppo_task_only/`。相对修订版仅将 dgppo_weight 设为 0 并更换输出目录，Safety Value 仍正常采样/训练，但不影响 Actor。两组使用相同任务优势归一化，不能把对照组称为完全未修改的原 MAPPO。
