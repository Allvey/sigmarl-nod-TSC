...
```bash
python main_training.py --config config_ppo_original_task_only.json
```

输出 `outputs/ppo_original_task_only/`。`dgppo_weight=0`，Safety Value 仍旁路采样/拟合，但不影响 Actor；任务优势保留原始值。

在相同 PPO 设置下启用 DGPPO：

```bash
python main_training.py --config config_ppo_original_dgppo.json
```

输出 `outputs/ppo_original_dgppo/`。与上一组只差 `dgppo_weight=1` 及输出目录。两组均从零训练，使用相同 seed、1 cm 道路余量、20 批安全预热、共享边界和原安全违规任务优势清零机制。
