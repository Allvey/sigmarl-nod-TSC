...
```bash
python main_training.py --config configs/archive/nod_history/config_dgppo_nod_fixed_finetune.json
```

- 初始化来源：`outputs/archive/ppo_original_task_only/reward7.36`。
- 输出：`outputs/archive/dgppo_nod_fixed_finetune/`。
...
```bash
python main_training.py --config configs/archive/nod_history/config_dgppo_nod_fixed.json
```

使用250批，输出至 `outputs/archive/dgppo_nod_fixed/`，不加载旧检查点。现有默认 `configs/archive/staged_history/config.json` 未切换。
