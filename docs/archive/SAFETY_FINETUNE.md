...
```bash
python main_training.py --config configs/archive/dgppo_history/config_ppo_original_dgppo_additive.json
```
...
新配置与下述微调配置仅模式和输出目录不同：同样从 reward7.36 初始化，70 批、前 20 次 Value 拟合冻结 PPO、安全权重 1、KL 阈值 0.01。输出到 `outputs/archive/ppo_original_dgppo_additive/`。评估时将模型和评估输出路径切换到此目录，沿用相同场景、步数和随机种子；对照原微调和源 task-only 的道路碰撞、车间碰撞及速度。
...
```bash
python main_training.py --config configs/archive/dgppo_history/config_ppo_original_dgppo_finetune.json
```

加载 `outputs/archive/ppo_original_task_only/reward7.36` 的 Actor、任务 Critic 和 Safety Value，输出到独立目录 `outputs/archive/ppo_original_dgppo_finetune/`。源权重不会覆盖。不要求不存在的 NOD 检查点；意见模块仍关闭。
