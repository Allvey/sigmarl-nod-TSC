...
> Current method note. Model paths and runnable commands are tracked in
...
Use `configs/archive/nod_history/config_dgppo_nod_responsibility_finetune.json` for this mode. It loads the
same-shaped NOD checkpoint but interprets evidence through the fixed mapping and
writes results to `outputs/dependencies/dgppo_nod_responsibility_finetune/`.

After the responsibility model has been trained, use
`configs/archive/nod_history/config_dgppo_nod_responsibility_control_finetune.json` for the control stage.
...
`configs/current/config_dgppo_nod_candidate_only.json` implements the controlled NOD stage. A
...
`outputs/dependencies/dgppo_nod_candidate_only/final_nod.pth`; the frozen companion networks
...
python -m scripts.validate_nod_responsibility \
  --checkpoint outputs/dependencies/dgppo_nod_candidate_only/final_nod.pth
```

After those cases pass, `configs/current/config_dgppo_nod_gain1_control_finetune.json` loads the
candidate, freezes NOD, and enables opinion-conditioned DGPPO with gain 1 and a
`|z| <= 0.1` fixed-alpha deadzone.

## Road-safe follow-up

`configs/archive/nod_history/config_dgppo_nod_gain1_road_safe_finetune.json` starts from the selected gain-1
...
```text
outputs/archive/dgppo_nod_gain1_road_safe_finetune/rule_vehicle_multiseed/
...
`outputs/dependencies/dgppo_nod_candidate_only/final` and retain identical optimization and
random-seed settings:

```bash
# Full mechanism: real z reaches both Actor and opinion-conditioned alpha.
python main_training.py --config configs/current/config_dgppo_nod_gain1_control_finetune.json

# Real z reaches Actor, but the DGPPO barrier always uses alpha=10.
python main_training.py --config configs/ablations/config_dgppo_nod_ablation_fixed_alpha.json

# Physical NOD context remains, but z is zeroed only at the Actor input;
# the DGPPO barrier also uses alpha=10.
python main_training.py --config configs/ablations/config_dgppo_nod_ablation_neutral_z.json
```
...
python -m scripts.run_rule_multiseed_evaluation \
  --model-path outputs/current/ablations/dgppo_nod_ablation_fixed_alpha/ \
  --seeds 101 202 303 404 505 --no-video
python -m scripts.run_rule_multiseed_evaluation \
  --model-path outputs/current/ablations/dgppo_nod_ablation_neutral_z/ \
  --seeds 101 202 303 404 505 --no-video
```
