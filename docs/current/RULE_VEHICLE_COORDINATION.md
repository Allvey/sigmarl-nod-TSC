...
`main_testing.py` 使用 `coordinated_rules_v7`。`rule_fraction`、类型比例和巡航速度仍在该文件开头设置。规则车之间可以共享位置、路线和动作计划；这些信息不添加到 Actor 或 NOD 的观测中。训练不使用这个控制器。
...
无交通干扰、实际 VMAS 动力学下，当前四条弯道平均速度从约 0.37–0.42 m/s 提高到 0.61–0.68 m/s，整条路线通过时间缩短约 19%–28%。六条路线的车身均未接触左右道路边界。对照记录见 `outputs/archive/rule_coordination_checks/free_route_speed_comparison.json`。
...
视频和日志保存到原检查点下的 `rule_vehicle_visualization/`，运行目录带有 `coordinated_rules_v7` 后缀。脚本会打印完整输出目录。
...
结果位于 `outputs/archive/rule_coordination_checks/`。验证脚本在规则车接触、最终预约不可行或持续低速超过阈值时返回失败。默认持续低速报警阈值是 15 秒、速度阈值是 0.03 m/s；它用于发现堵塞，不能作为数学上的无死锁证明。
