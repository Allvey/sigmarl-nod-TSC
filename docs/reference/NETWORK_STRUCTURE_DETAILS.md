...
本文档基于当前仓库源码和根目录 `configs/archive/staged_history/config.json`，整理项目中各个神经网络模块的结构、隐藏层、输入输出维度。
...
- 当前配置：`configs/archive/staged_history/config.json`
...
当前 `configs/archive/staged_history/config.json` 中的关键参数：
