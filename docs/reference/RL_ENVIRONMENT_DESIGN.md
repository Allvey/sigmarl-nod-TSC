...
本文档基于当前仓库代码与根目录 `configs/archive/staged_history/config.json`，梳理该项目中的强化学习智能体状态、动作、奖励机制，以及关键环境参数、训练参数、车辆参数和拓扑/对手建模相关参数。
...
当前 `configs/archive/staged_history/config.json` 中：
...
注意：`configs/archive/staged_history/config.json` 中写了 `n_agents = 6`，但 `CPM_mixed` 在 `utilities/constants.py` 中定义的 agent 数是 4。环境初始化时会根据 `scenario_type` 使用场景常量覆盖 agent 数，因此当前实际 agent 数应按 4 理解。
...
当前 `configs/archive/staged_history/config.json` 中训练参数：
...
1. `configs/archive/staged_history/config.json` 中 `n_agents = 6`，但 `CPM_mixed` 实际使用 4 个 agent。
